...
const ModalShowPost = ({ modalShowPostOpen, setModalShowPostOpen }) => {
    ...
    const fetchUser = async (userId) => {
        try {
            const res = await axiosInstance.get(getUserByIdAPI + userId + "/posts");
            setUsers(res.data);
        } catch (error) {
            console.log(error);
        }
    ...
    return (
        <>   
         {loading}      
            <Modal
                title="Post"
                open={modalShowPostOpen}
                footer={null}
                onCancel={() => {
                    setModalShowPostOpen(false);
                }}
            >
               <Card sx={{ maxHeight: '60%' }}>
                    ...
                    />
                    <CardContent>
                        <Typography gutterBottom variant="h2" component="div" color={'#3333ff'}>
                            {users.title}
                        </Typography>
